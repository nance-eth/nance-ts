...
async function main(){
  ...
  const { number: numberV2} = await treasury.getQueuedConfiguration('V2');
  const { number: numberV3, start: startV3, duration: durationV3 } = await treasury.getQueuedConfiguration('V3');
  ...
  //   await dolt.localDolt.commit('Update governanceCycles to GC38');
  //   await dolt.localDolt.push(`GC${cycleNumber}`);
  // });
}
